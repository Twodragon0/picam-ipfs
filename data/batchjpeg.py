...
# where to save extracted JPEG still frames
picDir = "jpg/"
# event log file to append to
logFileName = "RecSeq_log.csv"
...
def saveJPEG(fin, fout, otime):
    ...
    if not os.path.isfile(fout):  # check if we've already saved it previously
        ...
        call(["ffmpeg", "-ss", otime, "-i", fin,
              "-frames:v", "1", "-q:v", "2", fout])
...
def saveCrop(fname, geom):
    fin = picDir + fname
    fout = picDir + fname  # output file directory
    if not os.path.isfile(fout):  # check if we've already saved it previously
        #    print("%s already exists" % fout)
        #  else:
...
#  global dateStr # time/date string from file
    ...
    try:
        ...
        if (eventNow) and (pxls >= pThresh):  # this frame is in event, and has valid pixel count
            ...
            sumY = sumY + ycent
            # time in seconds (offset from t=0 of 30)
            xVec[eFrame] = float(row[1])
            # horiz. center position in pixels, 0..96
            yVec[eFrame] = float(row[4])
...
def scanFiles(fname):
    ...
    if fname.endswith(".txt") and fname[0:6].isdigit():
        #    print("file: %s" % fname)
        # construct video filename from index file
        vidfile = fname[:-3] + "mp4"
