...
global running  # have we done the initial array processing yetglobal vPause   # true when we should stop grabbing frames
...
picDir = "data/"   # where to store still frames
videoDir = "data/"   # where to store video files
tmpDir = "data/"  # where to store YUV frame buffer
...
# usually -6 is the right value for sunny days. Maybe 0 when rainy/cloudy
expCompensate = -7

# minimum preferred number of brightest pixels (if too few, increase exposure comp.)
hBrightMin = 1
# max count brightest pixels (if too many, reduce exposure comp)
hBrightMax = 25
# start assuming an OK count of bright pixels
hBright = (hBrightMin + hBrightMax)/2
...
def initMaps():
    ...
    # difference between new & avg
    difmap = np.zeros((ysize, xsize), dtype=np.float32)
    expMask = np.ones((ysize, xsize), dtype=np.float32)  # exposure mask

    # rolling average of pix values
    stavg = np.zeros((ysize, xsize), dtype=np.int32)
    # rolling average sum of squared pix values
    sqavg = np.zeros((ysize, xsize), dtype=np.int32)
    # rolling average standard deviation
    stdev = np.zeros((ysize, xsize), dtype=np.int32)
    # rolling average difference
    avgdif = np.zeros((ysize, xsize), dtype=np.int32)
...
def saveFrame(camera):
    if (not vPause):
        ...
        camera.capture(fname, format='jpeg', resize=(
            1280, 720), use_video_port=True)
...
def getFrame(camera):
    ...
    camera.capture(stream, format='yuv', resize=(
        xsize, ysize), use_video_port=True)
...
#    print("raw: %5.3f new: %5.3f" % (rawmap[1,1], newmap[1,1]))
    # find histogram of image data in range [0..255]
    hist, bin_edges = np.histogram(rawmap, bins=hBins, range=(0, 255))
    # running average of largest histogram bin (brightest pixels)
    hBright = (hBright * 0.95) + (0.05*hist[hBins-1])
    ...
    if (edgeAvg < 0.1):   # there will be trouble if edgeAvg == 0
        edgeAvg = 0.1
    # scale new image by this factor to cancel exposure change
    scaleFactor = bkgAvg / edgeAvg
...
#    print("SF: %5.3f" % scaleFactor) # DEBUG check what the scale factor is
    ...
    # mag. diff. of orig pixmap (amount of per-pixel change)
    difmapA = abs(newmap - stavg)
    # mag. diff. of scaled pixmap (amount of per-pixel change)
    difmapB = abs(snewmap - stavg)

    if not settled:
        # rolling avg of most recent 'stg' images (approximately)
        stavg = stavg + sti * (newmap - stavg)
        # rolling avg square of images (approx)
        sqavg = sqavg + sti * (np.power(newmap, 2) - sqavg)
    ...
    # novel pixels have difference exceeding (dFactor * standard.deviation)
    novelA = difmapA - (dFactor * stdev)
    novelB = difmapB - (dFactor * stdev)   # same but using rescaled pixmap

    # boolean array, 1 where pixel with positive novelty value exists
    conditionA = novelA > 0
    # make a list containing only changed pixels
    changedPixelsA = np.extract(conditionA, novelA)
    # boolean array, 1 where pixel with positive novelty value exists
    conditionB = novelB > 0
    # make a list containing only changed pixels
    changedPixelsB = np.extract(conditionB, novelB)
...
#    print(stavg)
    ...
    if (countPixelsA >= pixThresh):  # motion is detected
        # rolling avg of most recent 'stg' images (approximately)
        stavg = stavg + sti2 * (newmap - stavg)
        # rolling avg square of images (approx)
        sqavg = sqavg + sti2 * (np.power(newmap, 2) - sqavg)

    # variance, had better not be negative
    devsq = (stg * stg * sqavg) - np.power((stavg*stg), 2)
    # force all elements to have minimum value = 0.1
    np.clip(devsq, 0.1, 1E15, out=devsq)
    # adding 1.0 * pixvalScaleFactor is just saying every pixel has at least one count of std.dev
    # matrix holding rolling-average element-wise std.deviation
    stdev = pixvalScaleFactor + (1.0/stg) * np.power(devsq, 0.5)
...
#      print("* countPx: %d, %d" % (countPixelsA, countPixelsB))
    ...
    if (countPixelsA > countPixelsB):  # conservative detection: fewer motion pixels is the better choice
        ...
        if (countPixelsB > 0):  # found something! (at least one pixel's worth of something)
            # clipping to integer still leaves plenty of precision
            avgNovel = int(np.average(changedPixelsB))
            # force negative values to 0, and clip positive values to 1
            np.clip(novelB, 0.0, 1.0, out=novelB)
            moMap = ndimage.binary_dilation(novelB)
            novelC = ndimage.binary_erosion(moMap, iterations=2)
            # recount how many 'novel' pixels there are after dilation + erosion
            countPixels = np.count_nonzero(novelC)

        if (countPixels > 0):
            # (x,y) center of motion. x is horizontal axis on image
            (ycent, xcent) = ndimage.measurements.center_of_mass(novelB.astype(int))
            # generate bitmaps showing location of novel pixels
            if (debugMap) and (countPixels > pixThresh):
        ...
        if (countPixelsA > 0):  # found something! (at least one pixel's worth of something)
            # clipping to integer still leaves plenty of precision
            avgNovel = int(np.average(changedPixelsA))
            # force negative values to 0, and clip positive values to 1
            np.clip(novelA, 0.0, 1.0, out=novelA)
        ...
        if (countPixels > 0):
            # (x,y) center of motion. x is horizontal axis on image
            (ycent, xcent) = ndimage.measurements.center_of_mass(novelA.astype(int))
            # generate bitmaps showing location of novel pixels
            if (debugMap) and (countPixels > pixThresh):
                img = Image.fromarray(65535 * novelC.astype(int))
                fnumstr = "%03d" % fnum
...
#      print(B)
        (y0, x0), (y1, x1) = B.min(0), B.max(
            0) + 1  # coordinates of bounding box
    else:
...
def adjBright(camera):
    ...
    # update camera exposure compensation
    camera.exposure_compensation = expCompensate
...
class MyCustomOutput(object):
    ...
    def write(self, buf):
        ...
        # how many (I,P,P,P...) H264 stream frame sets we have seen
        global nGOP
        ...
        if (ftype != 2) and (okGo == True):  # ok to re-enable event detection
            vPause = False
        # ignore continuation of a previous frame, and SPS headers
        if (fnum != fnumOld) and (ftype != 2):

            trueFrameNumber = trueFrameNumber + 1
            # how many frames since start of this H264 segment (file)
            segFrameNumber = segFrameNumber + 1
            fnumOld = fnum
...
#    tStart = time.time() # seconds since Jan.1 1970
    ...
    # dummy file with program start-time
    log = open(videoDir+"PiMotionStart.log", 'w')
    log.close()
...
#    camera.meter_mode = 'backlit' # largest central metering region
    ...
    # usually, slightly darker than default
    camera.exposure_compensation = expCompensate
    # black rectangle behind white text for readibility
...
#      print("logfile = %s" % logFileName)
        ...
        # update exposure compensation every pass (30 second intervals)
        adjBright(camera)
        ...
        while (okGo == True):  # write callback turns off 'okGo' near end of final GOP
            ...
            if not vPause:
                ...
                if (countPixels >= pixThresh):
                    # number of seconds since start of current H264 segment
                    eventRelTime = time.time() - segTime
            ...
            # just for output formatting, so it will use at most 4 characters
            avgNovel = min(avgNovel, 9999)
